...
import json, os, datetime, time, string, random
...
@csrf_exempt
def ke_upload(req):
    ext_allowed = ['git', 'jpg', 'jpeg', 'png']
    max_size = 2621440
    today = datetime.datetime.today()
    save_dir = 'attached/%d/%d/%d/' % (today.year, today.month, today.day)
    ...
    if req.method == 'POST':
        ...
        new_file = '%s.%s' % (int(time.time() + random.randint(0, 999999)), ext)
...
def ke_man(req):
    ...
    order = string.lower(req.GET.get('order', 'name'))
